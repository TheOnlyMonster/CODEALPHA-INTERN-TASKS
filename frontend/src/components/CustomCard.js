...
export default function CustomCard({img_md, text_md, title_fontSize, description_fontSize, img_sm, text_sm, content}) {
  return (
    <Grid container spacing={2} padding={3} sx={{":hover": { cursor: "pointer", transform: "scale(1.03)" }, transition: "all 0.3s ease-in-out"}}>
      <Grid xs={12} sm={img_sm} md={img_md}>
        <AspectRatio sx={{ borderRadius: "lg" }} ratio={"4/3"}>
          <img
            src={getImageSrc(content.image)}
            alt={content.title}
          />
        </AspectRatio>
      </Grid>
